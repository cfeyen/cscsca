use crate::{rules::sound_change_rule::{Cond, LabelType, ScopeId}, meta_tokens::ScopeType};
use super::*;
...
fn apply_empty_rule_to_no_phones() {
    ...
    assert_eq!(Ok(()), apply(&rule, &mut Vec::new()));
}
...
fn apply_empty_rule_to_one_phone() {
    ...
    assert_eq!(Err(ApplicationError::MatchError(MatchError::EmptyInput)), apply(&rule, &mut vec![Phone::new("a")]));
}
...
fn one_to_one_shift() {
    ...
    assert_eq!(Ok(()), apply(&rule, &mut phones));
...
fn remove_word_final_ltr() { // also tests bound deduplication
    let rule = SoundChangeRule {
        ...
        output: vec![],
        conds: vec![Cond { before: Vec::new(), after: vec![RuleToken::Phone(Phone::new_bound())] }],
        anti_conds: Vec::new(),
    };
...
fn option_to_phone() { // also tests bound deduplication
    ...
    assert_eq!(Err(ApplicationError::MatchError(MatchError::EmptyInput)), apply(&rule, &mut phones));
}
...
fn quadruple_agreement() { // also tests bound deduplication
    let rule = SoundChangeRule {
        ...
        conds: vec![Cond { before: Vec::new(), after: vec![
            RuleToken::SelectionScope { id: Some(ScopeId::Name("label")), options: vec![
        ...
        anti_conds: vec![Cond { before: vec![
            RuleToken::SelectionScope { id: Some(ScopeId::Name("label")), options: vec![

...
fn fail_match_same_labeled_optional_scopes_where_the_first_could_match_but_the_second_could_not() {
    ...
    assert_eq!(Ok(false), tokens_match_phones_from_right(&tokens, &[Phone::Symbol("d"), Phone::Symbol("b"), Phone::Symbol("c")], &mut choices));
...
fn fail_match_gap_across_bound() {
    ...
    assert_eq!(Ok(false), tokens_match_phones_from_right(&tokens, &phones, &mut choices))
}
...
fn match_gap_to_bound() {
    ...
    let phones = [Phone::Symbol("a"), Phone::Symbol("b"), Phone::Bound];
    let mut choices = Choices::default();

    assert_eq!(Ok(true), tokens_match_phones_from_right(&tokens, &phones, &mut choices))
}